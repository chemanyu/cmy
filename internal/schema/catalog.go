...
var colMeta = map[string]Column{
	...
	"unikey":          {Type: "varchar(16)", Kind: KindDim, Desc: "监测ID，用于按监测查询曝光、点击、转化；不是逐条记录的唯一键，不据此做明细一对一 JOIN"},
	"advertiser_id":   {Type: "varchar(128)", Kind: KindDim, Desc: "账户 ID（advertiser_id），用于京东账户点击/转化查询；字符串，不能转为浮点数"},
	...
	"type":            {Type: "varchar(32)", Kind: KindDim, Desc: "记录类型/上报类型，京东排查关键字段；京东callback中scheduled_callback表示低活订单转化，即使up_event_name为空也属于低活订单；err非空表示已收到但被我方过滤、不向下游回传，仍计入收到订单量。其他未定义取值不猜测"},
	...
	"err":             {Type: "text", Kind: KindDiag, Desc: "过滤/错误原因。通用track和京东callback中err非空表示转化已真实收到，但被我方过滤、不向下游回传；收到量应包含这些记录。err为空（NULL或空串）仅表示未标记过滤，不保证回传成功。排查时按err分组"},
	...
	"up_event_name":      {Type: "varchar(128)", Kind: KindDim, Desc: "上游转化事件；京东事件4用字符串值4过滤。京东callback的低活订单由type=scheduled_callback识别，up_event_name可能为空；全部订单需同时覆盖两种情况"},
	"down_event_name":    {Type: "varchar(128)", Kind: KindDim, Desc: "下游（广告主侧）事件名"},
	"action_pv":          {Type: "tinyint", Kind: KindMetric, Desc: "浅层转化行为数；收到转化行为量按全部记录累加，包含err非空的我方过滤记录；不等于下游回传量"},
