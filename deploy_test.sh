...
# 配置信息（内网测试机）
REMOTE_USER="root"
REMOTE_HOST="172.16.3.34"
REMOTE_DIR="/data/ocpx-mcp"
...
# 测试机部署路径不是 /usr/local/adt-go/ocpx_mcp，service 里的路径要换掉
info "同步文件到服务器..."
sed "s#/usr/local/adt-go/ocpx_mcp#$REMOTE_DIR#g" $SERVICE_FILE > /tmp/$SERVICE_FILE.test || error "渲染 service 失败"
scp $LOCAL_BINARY $REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/$TEMP_BINARY || error "文件同步失败"
scp /tmp/$SERVICE_FILE.test $REMOTE_USER@$REMOTE_HOST:/etc/systemd/system/$SERVICE_FILE || error "服务配置文件同步失败"
rm -f /tmp/$SERVICE_FILE.test

# config.yaml 里有 Doris 密码和 MCP token：首次部署上传模板，之后一律保留远端的不动
NEED_CONFIG_EDIT=false
if ssh $REMOTE_USER@$REMOTE_HOST "test -f $REMOTE_DIR/$CONFIG_FILE"; then
    info "$CONFIG_FILE 已存在，保留不动"
else
